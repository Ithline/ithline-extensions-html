...
public sealed class HtmlNodeTest
{
    ...
    public void Parse_WhenContainsBody_ReturnsDivElement()
    {
        ...
        body.Should().BeEquivalentTo(expectation, o => o.IncludingAllRuntimeProperties());
    }

    [Fact]
    public void Parse_WhenNodeContainsWhiteSpaceText_ReturnsOriginalText()
    {
        // arrange
        var text = "<div><span>  some text -</span><span> again some text.</span></div>";

        // act
        var node = HtmlNode.Parse(text);

        // assert
        var expectation = HtmlNode.Element("div", children: new HtmlNode[]
        {
            HtmlNode.Element("span", children: new HtmlNode[] { HtmlNode.Text("  some text -") }),
            HtmlNode.Element("span", children: new HtmlNode[] { HtmlNode.Text(" again some text.") }),
        });
        node.Should().BeEquivalentTo(expectation, o => o.IncludingAllRuntimeProperties());
    }
}
